import { createRunOncePlugin, withAndroidManifest, withAndroidStyles, } from '@expo/config-plugins';
...
function addActivityToManifest(config) {
    return withAndroidManifest(config, async (newConfig) => {
        const mainApplicationActivities = newConfig.modResults.manifest.application?.[0]?.activity;
        const hasScreenGuardActivity = mainApplicationActivities?.some((activity) => activity.$['android:name'] ===
 ...
 * @param config
 */
function addScreenGuardThemeToStylesXml(config) {
    return withAndroidStyles(config, async (newConfig) => {
        const styles = newConfig.modResults.resources.style || [];
        const hasScreenGuardTheme = styles?.some((style) => style.$.name === 'Theme.AppCompat.Translucent');
...
export default createRunOncePlugin(withScreenGuardAndroid, pkg.name, pkg.version);
